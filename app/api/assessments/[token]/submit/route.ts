...
export async function POST(req: NextRequest, { params }: { params: { token: string } }) {
  ...
  if (!assessment.passage_audio_url || !assessment.speaking_audio_url) {
  ...
  try {
    const [passageFile, speakingFile] = await Promise.all([
    ...
    if (passageFile.error || speakingFile.error) {
      throw new Error("Could not read recordings from storage");
    }

    const [passageBuf, speakingBuf] = await Promise.all([
      passageFile.data.arrayBuffer(),
      speakingFile.data.arrayBuffer()
    ]);

    const [passageTranscription, speakingTranscription] = await Promise.all([
    ...
    const speakingDuration =
      speakingTranscription.words.length > 0
        ? speakingTranscription.words[speakingTranscription.words.length - 1].end -
          speakingTranscription.words[0].start
        : 0;
    ...
    const provisional = runScoringEngine({
      ...
      speakingWords: speakingTranscription.words,
