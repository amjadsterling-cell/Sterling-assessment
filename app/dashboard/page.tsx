...
function StatCard({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="relative bg-black/40 rounded-xl p-5 border border-white/10 overflow-hidden">
      <div className="absolute top-0 left-0 right-0 h-[3px] bg-brand-gradient" />
      <p className="text-xs text-gray-400 label uppercase tracking-wide">{label}</p>
      <p className="text-3xl font-heading font-bold mt-2 text-white">{value}</p>
    </div>
  );
}
